...
export function WebSocketProvider({ children }) {
  ...
  // Same pattern as subscribeToGroup above, but for the dedicated
  // /topic/group-call/{id} channel GroupCallWsController broadcasts
  // JOIN/LEAVE/END roster events on.
  const groupCallSubsRef = useRef({});
  const pendingGroupCallsRef = useRef(new Set());
  const subscribeToGroupCall = useCallback((groupId) => {
    if (!groupId) return;
    const id = String(groupId);
    if (groupCallSubsRef.current[id]) return;

    if (clientRef.current?.connected) {
      const sub = clientRef.current.subscribe(`/topic/group-call/${id}`, frame => {
        try {
          const msg = JSON.parse(frame.body);
          listenersRef.current.forEach(fn => fn(msg));
        } catch { }
      });
      groupCallSubsRef.current[id] = sub;
    } else {
      pendingGroupCallsRef.current.add(id);
    }
  }, []);
  const unsubscribeFromGroupCall = useCallback((groupId) => {
    if (!groupId) return;
    const id = String(groupId);
    groupCallSubsRef.current[id]?.unsubscribe?.();
    delete groupCallSubsRef.current[id];
    pendingGroupCallsRef.current.delete(id);
  }, []);
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect() {
        ...
        const allPendingCalls = new Set([
          ...pendingGroupCallsRef.current,
          ...Object.keys(groupCallSubsRef.current),
        ]);
        groupCallSubsRef.current = {};
        pendingGroupCallsRef.current.clear();
        allPendingCalls.forEach(id => {
          const sub = client.subscribe(`/topic/group-call/${id}`, frame => {
            try {
              const msg = JSON.parse(frame.body);
              listenersRef.current.forEach(fn => fn(msg));
            } catch { }
  ...
  return (
    <WebSocketContext.Provider value={{ publish, subscribe, subscribeToGroup, subscribeToGroupCall, unsubscribeFromGroupCall, subscribeToChannel }}>
      {children}
