...
export function CallProvider({ children }) {
  ...
  // "Add to call" — see mobile CallContext.js's upgradeToGroup() for the
  // full explanation (same design on both platforms): converts this 1:1
  // P2P call into a multi-party mesh room, with a brief reconnect since
  // the 1:1 connection isn't mesh-capable. upgradingToRoomId rides along
  // on the normal /call.end frame — relay() forwards arbitrary extra
  // fields as-is, no backend change needed. GroupCallContext (mounted
  // alongside this one) watches for that flag on CALL_END and auto-joins
  // instead of just hanging up.
  const upgradeToGroup = useCallback((roomId) => {
    const current = statusRef.current;
    if (current !== 'active' && current !== 'calling') return;
    const targetId = remoteUserRef.current?.userId;
    if (targetId) {
      wsRef.current?.publish?.('/app/call.end', {
        targetId, senderId: userId, callId: activeCallIdRef.current,
        upgradingToRoomId: roomId, upgradingCallType: callTypeRef.current,
      });
    }
    resetState();
  }, [userId]);
  ...
  const handleMsg = useCallback((msg) => {
    ...
    // Group-call mesh links reuse these same CALL_OFFER/ANSWER/ICE/END
    // message types for pairwise WebRTC negotiation between each pair of
    // participants (see GroupCallContext.js) — tagged with a groupId so
    // this 1:1 handler ignores them instead of misreading a mesh-link
    // offer as an incoming 1:1 call.
  ...
  const value = {
    ...
    startCall, acceptCall, declineCall, endCall, upgradeToGroup, toggleMute, toggleVideo,
  };
