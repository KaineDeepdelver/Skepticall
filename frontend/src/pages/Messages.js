...
/* ── BUBBLE ── */
// WhatsApp-style call-log row — "X started a call" with a Join button
// while the room's still open, "Call ended" once it's not. Centered,
// unlike normal bubbles, since it's not "from" anyone in particular the
// way a message is — it's a system-style event about the conversation.
function CallLogRow({ msg, onJoin }) {
  const timeStr = fmtTime(msg.sentAt ?? msg.createdAt);
  const isVideo = msg.content === 'video';
  const ended = msg.callStatus === 'ENDED';
  return (
    <div style={{ display: 'flex', justifyContent: 'center', margin: '10px 0' }}>
      <div style={{
        display: 'flex', alignItems: 'center', gap: 10,
        background: 'var(--bg-hover)', border: '1px solid var(--border)',
        borderRadius: 14, padding: '10px 14px', maxWidth: 340,
      }}>
        <div style={{
          width: 32, height: 32, borderRadius: '50%', flexShrink: 0,
          background: ended ? 'var(--bg-hover)' : 'var(--accent)',
          border: ended ? '1px solid var(--border)' : 'none',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: ended ? 'var(--text-muted)' : 'var(--accent-text)',
        }}>
          {isVideo
            ...
            : <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14"><path d="M22 16.92v3a2 2 0 0 1-2.18 2A19.79 19.79 0 0 1 11.39 18a19.5 19.5 0 0 1-3.39-3.39A19.79 19.79 0 0 1 2.12 6.18 2 2 0 0 1 4.11 4h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 11.91a16 16 0 0 0 4 4l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 20 16z" /></svg>}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {msg.senderDisplayName || msg.senderUsername || 'Someone'} started a {isVideo ? 'video' : 'voice'} call
          </span>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{ended ? 'Call ended · ' : ''}{timeStr}</span>
        </div>
        {!ended && (
          <button
            onClick={() => onJoin(msg)}
            style={{
              marginLeft: 4, background: 'var(--accent)', color: 'var(--accent-text)',
...
function Bubble({ msg, isSent, onContextMenu, isGroup, groupCreatorId, selectMode, isSelected, onSelect }) {
  ...
  if (isDeleted) { inner = <><span className="bubble-deleted">⊘ This message was deleted</span><span className="bubble-gap short" /><span className="bubble-footer"><span className="bubble-time">{timeStr}</span></span></>; }
    ...
    inner = (
      <span className={`bubble-call${isFailedCall ? ' bubble-call-failed' : ''}`}>
...
export default function Messages() {
  ...
  const { startGroupCall, peekRoom, roomPeek, autoJoinRoom } = useGroupCall();
  // Join button on a call-log row — reuses the same silent-join path as
  // a 1:1-call-upgrading-to-group (autoJoinRoom), since joining from
  // chat history should just drop you into the room, no ringing state.
  const handleJoinCallLog = useCallback((msg) => {
    autoJoinRoom(msg.groupId, msg.content).catch(e => console.error('[Messages] join call log:', e));
  }, [autoJoinRoom]);
  ...
  const handleWsMessage = (msg) => {
    ...
    // WebRTC/call signaling (1:1 calls via CallContext, group calls via
    // GroupCallContext) is handled exclusively by those contexts, which
    // are subscribed to this same shared listener registry independently
    // of which topic actually delivered the frame — so every one of these
    // also reaches this handler and must be dropped here, or it leaks
    // into the message list as a blank bubble (it has a groupId but isn't
    // a chat message). Matched by prefix instead of an explicit list so
    // new signaling types (this bit us before with GROUP_CALL_INVITE/
    // GROUP_CALL_END, added after this filter and not included in it)
    // are covered automatically. The persisted "CALL" call-log message
    // (WhatsApp-style "X started a call" row) is a real chat message and
    // deliberately doesn't match any of these prefixes.
    if (msgType && (msgType.startsWith('CALL_') || msgType.startsWith('GROUP_CALL_') || msgType.startsWith('VOICE_'))) return;
    ...
    if (msg.groupId) {
      ...
      if (activeConvoRef.current && Number(activeConvoRef.current.groupId) === Number(msg.groupId)) {
        ...
        // Call-log row (see logCallStarted/logCallEnded on the backend) —
        // broadcast once with callStatus ONGOING when the call is rung,
        // then again with callStatus ENDED once it's torn down. Same id
        // both times, so this is an upsert: insert if we haven't seen
        // this id yet, otherwise just refresh callStatus on the existing
        // bubble (drops the Join button once ended). Falls through to the
        // shared bumpConvo call below like a normal message.
        if (msg.type === 'CALL') {
          setMessages(prev => {
            const idx = prev.findIndex(m => m.id === msg.id);
            if (idx !== -1) return prev.map((m, i) => i === idx ? { ...m, callStatus: msg.callStatus } : m);
            return [...prev, msg];
          });
          if (String(msg.callStatus) !== 'ONGOING') return; // ended — no preview bump needed
        } else {
        setMessages(prev => {
      ...
      if (msg.type === 'CALL' && msg.callStatus !== 'ONGOING') {
        // Ended-call update — bubble already upserted above if the convo
        // was open; nothing new to bump the conversation preview with.
      } else if (msg.type !== 'DELETE' && msg.type !== 'EDIT') {
        const preview = msg.type === 'CALL'
          ? `${msg.senderDisplayName || 'Someone'} started a call`
          : (msg.senderDisplayName ? `${msg.senderDisplayName}: ${msg.content || '[attachment]'}` : msg.content || '[attachment]');
        bumpConvo(gKey, preview);
      }
      return;
    }
    const type = msg.type;
  ...
  // Anyone can join a group call room quietly at any time — this just
  // checks "is there one open right now" so the header can show a "N in
  // call" indicator. See the "Ring everyone" button inside the call
  // itself for explicitly notifying the group.
  useEffect(() => {
    if (!activeConvo?.isGroup || !activeConvo?.groupId) return;
    peekRoom(activeConvo.groupId);
    const t = setInterval(() => peekRoom(activeConvo.groupId), 15000);
    return () => clearInterval(t);
  }, [activeConvo?.isGroup, activeConvo?.groupId, peekRoom]);
  ...
  async function sendMessage() {
    ...
    if (activeConvo.isGroup && activeConvo.groupId) {
      ...
      const payload = { groupId: activeConvo.groupId, content, type: 'TEXT', _tmpId };
      if (replyingTo) {
        optimistic.replyToId = replyingTo.id;
        optimistic.replyPreview = replyingTo.content?.slice(0, 200);
        optimistic.replyPreviewSender = replyingTo.senderName;
        payload.replyToId = replyingTo.id;
        payload.replyPreview = optimistic.replyPreview;
        payload.replyPreviewSender = replyingTo.senderName;
      }
      scheduleClearJustArrived(_tmpId);
      setMessages(prev => [...prev, optimistic]);
      send('/app/group.message', payload);
    } else {
  ...
  function submitEdit(msgId) {
    ...
    if (activeConvo?.isGroup) {
      send('/app/group.message.edit', { messageId: msgId, groupId: activeConvo.groupId, content: editText });
    } else {
  ...
  function renderMessages() {
    ...
    while (i < messages.length) {
      ...
      if ((msg.type || '').toUpperCase() === 'CALL') {
        items.push(<CallLogRow key={msg.id} msg={msg} onJoin={handleJoinCallLog} />);
        i++;
        continue;
      }
  ...
  return (
    <div className={`messages-layout${activeConvo ? ' mobile-chat-open' : ''}`} style={{ flex: 1 }}>
              ...
              <div className="chat-header-info" style={{ flex: 1, cursor: 'pointer' }} onClick={() => { if (!activeConvo.isGroup) navigate(`/profile/${activeConvo.username}`); else setViewingGroupInfo(activeConvo); }}>
                ...
                {activeConvo.isGroup && (
                  <span className="chat-header-status">
                    {(roomPeek.get(Number(activeConvo.groupId))?.length || 0) > 0
                      ? `${roomPeek.get(Number(activeConvo.groupId)).length} in call · click video/phone to join`
                      : `${activeConvo.members?.length || 0} members`}
                  </span>
                )}
              </div>

              {/* Call buttons — now available for groups too (room-based:
                  quietly joins/creates the ad-hoc group call room, doesn't
                  ring anyone — see the "Ring everyone" button inside the
                  call itself for that). Search stays 1:1-only. */}
              <div style={{ display: 'flex', gap: 6, marginRight: 6 }}>
                <button
                  onClick={() => {
                    if (!activeConvo) return;
                    if (activeConvo.isGroup) startGroupCall({ groupId: activeConvo.groupId, name: activeConvo.name }, 'audio');
                    else startCall('audio', activeConvo);
                  }}
                  title="Voice call"
                ...
                <button
                  onClick={() => {
                    if (!activeConvo) return;
                    if (activeConvo.isGroup) startGroupCall({ groupId: activeConvo.groupId, name: activeConvo.name }, 'video');
                    else startCall('video', activeConvo);
                  }}
                  title="Video call"
                  style={{ background: 'none', border: '1px solid var(--border-input)', borderRadius: '50%', width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--text-muted)', transition: 'all 0.15s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--accent)'; e.currentTarget.style.color = 'var(--accent-text)'; e.currentTarget.style.borderColor = 'var(--accent)'; }}
      ...
      {contextMenu && <ContextMenu
        ...
        onReply={() => {
          const isSent = contextMenu.msg.senderId === userId;
          const senderName = isSent
            ? (user?.displayName || user?.username || 'You')
            // For groups, the quoted message could be from any member, not
            // just "the other person" — use its own sender name. Falls
            // back to activeConvo?.name (the DM contact) when this isn't
            // a group message.
            : (contextMenu.msg.senderDisplayName || contextMenu.msg.senderUsername || activeConvo?.name || 'Them');
          setReplyingTo({ id: contextMenu.msg.id, content: contextMenu.msg.content || '[attachment]', senderName });
        }}
