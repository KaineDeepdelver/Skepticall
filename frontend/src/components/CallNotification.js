...
export default function CallNotification() {
  const {
    ...
    acceptCall, declineCall, endCall, upgradeToGroup,
  ...
  async function openAddModal() {
    ...
    try {
      ...
      setAddCandidates((convos || []).filter(c => !c.isGroup && String(c.userId) !== String(remoteUser?.userId)));
    } catch (e) {
      console.error('[CallNotification] openAddModal:', e);
      setAddCandidates([]);
    } finally {
      setAddLoading(false);
    }
  ...
  // Converts this 1:1 call into an ad-hoc group room containing both
  // original parties plus whoever was just picked — see CallContext.js's
  // upgradeToGroup() for why this can't be done without a brief
  // reconnect (the 1:1 call's peer connection isn't mesh-capable).
  function handleAddPerson(person) {
    setAddModalOpen(false);
    const roomId = Date.now();
    const roomType = callType;
    upgradeToGroup(roomId);
    startGroupCall({ groupId: roomId, name: null }, roomType);
    ws?.publish?.('/app/call.adhoc.invite', {
      targetId: person.userId, roomId, type: roomType,
  ...
  return (
    <>
          ...
          <div style={{ padding: '18px 20px 20px' }}>
            ...
            {isActive && (
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', gap: 14 }}>
                ...
                {/* Add to call — converts this 1:1 call into an ad-hoc
                    group room */}
                <CtrlBtn
                  ...
                  label="Add"
                  icon={<PlusIcon color="rgba(255,255,255,0.8)" size={18} />}
                />
      ...
      {addModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 10001, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setAddModalOpen(false)}>
          <div style={{ width: 340, maxWidth: '90vw', borderRadius: 18, background: '#15171d', border: '1px solid rgba(255,255,255,0.1)', padding: 16 }} onClick={e => e.stopPropagation()}>
            <div style={{ color: '#fff', fontSize: 15, fontWeight: 700, marginBottom: 12 }}>Add to call</div>
            {addLoading ? (
              <div style={{ color: 'rgba(255,255,255,0.4)', textAlign: 'center', padding: '24px 0' }}>Loading…</div>
            ) : addCandidates.length === 0 ? (
              <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, textAlign: 'center', padding: '24px 0' }}>No one else to add</div>
            ) : (
              <div style={{ maxHeight: 360, overflowY: 'auto' }}>
                {addCandidates.map(c => (
                  <div
                    key={c.userId}
                    onClick={() => handleAddPerson(c)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 4px', cursor: 'pointer', borderRadius: 8 }}
                    onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'none'; }}
                  >
                    <div style={{ width: 38, height: 38, borderRadius: 19, background: '#2a2d36', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 15, fontWeight: 700, flexShrink: 0, overflow: 'hidden' }}>
                      {c.avatar ? <img src={c.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (c.name || c.username || '?').slice(0, 1).toUpperCase()}
                    </div>
                    <span style={{ color: '#fff', fontSize: 14, fontWeight: 600 }}>{c.name || c.username}</span>
                  </div>
                ))}
              </div>
            )}
            <button
              ...
              style={{ marginTop: 10, width: '100%', padding: '10px 0', background: 'none', border: 'none', color: 'rgba(255,255,255,0.55)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
            >
              Cancel
            </button>
          </div>
