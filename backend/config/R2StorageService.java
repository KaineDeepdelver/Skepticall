...
public class R2StorageService {
    ...
    public String upload(MultipartFile file, String prefix) {
        try {
            ...
            return uploadBytes(file.getBytes(), file.getContentType(), orig, prefix);
        } catch (IOException e) {
     ...
     * raw bytes instead of a MultipartFile — e.g. VideoTrimService's
     * ffmpeg output, which exists only as a temp file's bytes by the time
     * it's ready to store.
     */
    public String uploadBytes(byte[] rawBytes, String contentType, String originalFilename, String prefix) {
        try {
            ...
            MediaMetadataScrubber.ScrubResult scrubbed =
                    metadataScrubber.scrub(rawBytes, originalFilename, contentType);
